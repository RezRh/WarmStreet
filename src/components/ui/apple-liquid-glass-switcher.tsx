...
export function ThemeSwitcher(props: ThemeSwitcherProps) {
  const initialValue = props.value ?? props.defaultValue ?? "light";
  const [internalValue, setInternalValue] = createSignal<Theme>(initialValue);
  ...
  return (
    <fieldset
      class="switcher liquid-nav"
      data-active={activeValue()}
    >
      <legend class="sr-only">Choose theme</legend>

      {themeOptions.map((option) => (
        <label class="switcher__option liquid-item cursor-pointer">
          <input
            class="sr-only"
            type="radio"
            name="theme"
          ...
          />
          <div class={`transition-all duration-300 ${activeValue() === option.value ? 'active' : ''}`}>
            {option.icon}
          </div>
        </label>
      ))}
